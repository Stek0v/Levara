...
func buildTestGraph() *Graph {
	...
	// Nodes
	g.AddNode("n1", "Cognevra", "Vector database", "Software", "")
	...
	// Edges
	g.AddEdge("n1", "n2", "uses", "Cognevra uses HNSW for indexing", "etype_1")
	g.AddEdge("n1", "n3", "has", "Cognevra has WAL for durability", "etype_2")
	g.AddEdge("n1", "n4", "acceleratedBy", "Cognevra accelerated by SIMD", "etype_3")
	g.AddEdge("n5", "n2", "uses", "LanceDB uses different index", "etype_1")
	g.AddEdge("n1", "n5", "competesWith", "Cognevra competes with LanceDB", "etype_4")
...
func TestFormatTriplets(t *testing.T) {
	...
	if !strings.Contains(formatted, "Cognevra") {
		t.Error("formatted output missing node name 'Cognevra'")
	}
