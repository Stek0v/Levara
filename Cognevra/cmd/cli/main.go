// cognevra CLI — command-line interface for the Cognevra REST API.
//
// Usage:
//
//	cognevra health [--details]
//	cognevra add <file|url|text> [--dataset=name]
//	cognevra cognify [--dataset=name] [--collection=name] [--wait]
//	cognevra search <query> [--type=CHUNKS] [--top-k=10]
//	cognevra datasets list
//	cognevra datasets create <name>
//	cognevra datasets delete <id>
//	cognevra cache stats
//
// Configuration:
//
//	COGNEVRA_URL   API base URL (default http://localhost:8080/api/v1)
//	COGNEVRA_TOKEN Auth token (or ~/.cognevra/token file)
package main
...
func main() {
	baseURL = envOr("COGNEVRA_URL", "http://localhost:8080/api/v1")
	token = os.Getenv("COGNEVRA_TOKEN")
	...
	if token == "" {
		if home, err := os.UserHomeDir(); err == nil {
			if data, err := os.ReadFile(filepath.Join(home, ".cognevra", "token")); err == nil {
	...
	case "help", "--help", "-h":
		printUsage()
	default:
		fatalf("unknown command: %s\nRun 'cognevra help' for usage.", cmd)
	}
}
...
func cmdAdd(args []string) {
	...
	if len(positional) == 0 {
		fatalf("usage: cognevra add <file|url|text> [--dataset=name]")
	}
	input := positional[0]
...
func cmdSearch(args []string) {
	...
	if len(positional) == 0 {
		fatalf("usage: cognevra search <query> [--type=CHUNKS] [--top-k=10]")
	}
...
func cmdDatasets(args []string) {
	...
	case "create":
		if len(args) == 0 {
			fatalf("usage: cognevra datasets create <name>")
		}
		datasetsCreateCmd(args[0])
	case "delete":
		if len(args) == 0 {
			fatalf("usage: cognevra datasets delete <id>")
		}
		datasetsDeleteCmd(args[0])
	default:
		fatalf("unknown datasets subcommand: %s\nUsage: cognevra datasets [list|create <name>|delete <id>]", sub)
	}
}
...
func cmdCache(args []string) {
	if len(args) == 0 || args[0] != "stats" {
		fatalf("usage: cognevra cache stats")
	}
...
func printUsage() {
	fmt.Print(`Usage: cognevra <command> [flags]
...
Global flags:
  ...
  --token=<token>  Auth token (default: $COGNEVRA_TOKEN or ~/.cognevra/token)
