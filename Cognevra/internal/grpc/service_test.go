...
func startTestServer(t *testing.T, dim int) (pb.CognevraServiceClient, func()) {
	...
	dir, _ := os.MkdirTemp("", "cognevra-grpc-test-*")
	...
	db, _ := store.NewCognevra(dim, dbPath)
...
func BenchmarkGRPCSearch(b *testing.B) {
	dir, _ := os.MkdirTemp("", "cognevra-grpc-bench-*")
	...
	os.MkdirAll(fmt.Sprintf("%s/dummy/shard_0", dir), 0755)
	db, _ := store.NewCognevra(dim, fmt.Sprintf("%s/dummy/shard_0/meta.bin", dir))
	defer db.Close()
