...
func setupDB(t testing.TB, n, dim int) (*Cognevra, func()) {
	dir, err := os.MkdirTemp("", "cognevra-bench-*")
	if err != nil {
		t.Fatal(err)
	}
	db, err := NewCognevra(dim, dir+"/meta.bin")
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < n; i++ {
...
func BenchmarkInsertOneByOne(b *testing.B) {
	const dim = 64
	dir, _ := os.MkdirTemp("", "cognevra-bench-*")
	defer os.RemoveAll(dir)
	db, _ := NewCognevra(dim, dir+"/meta.bin")
