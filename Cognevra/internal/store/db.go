// Package store implements Cognevra's storage engine: HNSW index, WAL, arena, and disk store.
//
// The main entry point is [NewCognevra], which returns a [Cognevra] instance ready for
...
// VectroRecord is a single result returned by [Cognevra.Search].
...
// Cognevra is the core storage engine combining an in-memory HNSW index with a
...
//
// On startup, NewCognevra replays the WAL to restore all previously inserted records
// and starts a background goroutine for async HNSW indexing.
func NewCognevra(dim int, storagePath string, cfg ...HNSWConfig) (*Cognevra, error) {
...
func (db *Cognevra) signalIndexer() {
	select {
...
func (db *Cognevra) insertInMemory(id string, vector []float32, loc FileLocation) error {
	idx, err := db.arena.Add(vector)
	if err != nil {
		return err
	}
...
func (db *Cognevra) BatchInsert(records []BatchItem) []error {
...
func (db *Cognevra) Get(id string) ([]float32, []byte, bool) {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (db *Cognevra) Search(query []float32, topK int) []VectroRecord {
...
func (db *Cognevra) Delete(id string) error {
	db.mu.Lock()
...
func (db *Cognevra) BatchDelete(ids []string) []error {
	var errs []error
	for _, id := range ids {
