...
- **2026-04-15** — **DeepSeek response_format limitation**: при `UseStructuredOutput=true + LLMEndpoint="https://api.deepseek.com/v1"` API отвечает HTTP 400 `"This response_format type is unavailable now"` на `json_schema` формат. `llm.StructuredCall` retry-branch делает graceful fallback к plain JSON ответу на retry 1/3 — тест проходит, но это лишний round-trip (+2-3s). Для DeepSeek production-пути имеет смысл либо отключить `UseStructuredOutput`, либо в `llm/structured.go` учесть endpoint и сразу идти в plain-JSON режим. Найдено `TestPipeline_DeepSeek_StructuredOutput`.
