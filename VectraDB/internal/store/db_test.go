...
func TestDeleteByID(t *testing.T) {
	dir, _ := os.MkdirTemp("", "vectra-delete-test-*")
	defer os.RemoveAll(dir)

	db, err := NewVectraDB(64, dir+"/meta.bin")
	if err != nil {
		t.Fatalf("NewVectraDB: %v", err)
	}
	defer db.Close()
...
func TestDeleteWALRecovery(t *testing.T) {
	dir, _ := os.MkdirTemp("", "vectra-wal-del-test-*")
	defer os.RemoveAll(dir)

	dbPath := dir + "/meta.bin"

	// Phase 1: Insert + Delete
	db, _ := NewVectraDB(64, dbPath)
	vec := randVecForTest(64)
...
func TestBatchDelete(t *testing.T) {
	dir, _ := os.MkdirTemp("", "vectra-batch-del-test-*")
	defer os.RemoveAll(dir)

	db, _ := NewVectraDB(64, dir+"/meta.bin")
	defer db.Close()
