...
func setupDB(t testing.TB, n, dim int) (*VectraDB, func()) {
	dir, err := os.MkdirTemp("", "vectradb-bench-*")
	if err != nil {
		t.Fatal(err)
	}
	db, err := NewVectraDB(dim, dir+"/meta.bin")
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < n; i++ {
...
func BenchmarkInsertOneByOne(b *testing.B) {
	const dim = 64
	dir, _ := os.MkdirTemp("", "vectradb-bench-*")
	defer os.RemoveAll(dir)
	db, _ := NewVectraDB(dim, dir+"/meta.bin")
