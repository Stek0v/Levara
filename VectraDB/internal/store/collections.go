...
// CollectionManager manages multiple VectraDB instances, one per collection.
...
// Get returns the VectraDB instance for a collection.
...
func (cm *CollectionManager) getOrCreate(name string) (*VectraDB, error) {
	cm.mu.RLock()
	db, exists := cm.collections[name]
	cm.mu.RUnlock()
