...
Defined in `VectraDB/proto/vectradb.proto`. Service: `VectraDBService` on port 50051.
...
The Python adapter (`VectraDBAdapter.py`) implements all 9 Cognee `VectorDBInterface` methods by translating to these RPCs. Transport overhead is approximately **0.3ms** round-trip (vs 2.6ms for the previous HTTP/JSON adapter, an 8.4x reduction).
