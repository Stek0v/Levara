...
доступна через защищённый REST API и CLI по immutable user/group ID; WebUI
получает document-scoped active recipients только после проверки share-права.
Настройки и проверяемые
