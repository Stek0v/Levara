# Новые UI компоненты Cognevra

## Обзор

В рамках текущей итерации реализовано 6 новых UI-компонентов для фронтенда Cognevra. Цель — предоставить полноценный графический интерфейс для функций, которые ранее были доступны только через API: шаринг датасетов, управление онтологиями и коллекциями, поиск по knowledge graph, индикация прогресса обработки и навигация по MCP-статусу. По сравнению с эталонным Cognee Python, Cognevra опережает по 3 из 6 компонентов и находится в паритете по остальным.
...
| Компонент | Cognee Python | Cognevra | Результат |
|-----------|--------------|----------|-----------|
| Share UI | Нет (только API) | Есть (модаль) | Cognevra опережает |
| Ontology UI | Нет (только demo-граф) | Есть (upload/list/delete) | Cognevra опережает |
| Collections UI | Нет (только status) | Есть (CRUD + re-embed) | Cognevra опережает |
...
По 3 из 6 компонентов Cognevra предоставляет функционал, отсутствующий в Cognee Python. Остальные 3 компонента находятся на уровне паритета с различиями в реализации (CSS modules vs Tailwind).
