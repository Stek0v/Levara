...
async def create_chunk_associations(
    ...
    for chunk_id, chunk_text in id_to_text.items():
        ...
        for candidate in candidates:
            ...
            candidate_pairs.append((chunk_id, chunk_text, candidate_id, candidate_text))

    async def _process_pair(chunk_id, chunk_text, candidate_id, candidate_text):
        similarity = await _compare_chunks(
            chunk_text, candidate_text, user_prompt_location, system_prompt_location
        )
        return (chunk_id, candidate_id, similarity)

    tasks = [_process_pair(cid, ct, aid, at) for cid, ct, aid, at in candidate_pairs]
    results = await gather_with_concurrency(*tasks)

    for chunk_id, candidate_id, similarity in results:
        if (
            similarity
