...
- **Levara** — наш production-оптимизированный продукт, соединяющий оба проекта: нативный gRPC транспорт (замена HTTP/JSON, снижение задержки в 8.4x), SIMD AVX2 (8.1x ускорение), WAL group commit (12.5x меньше fsync), нативная изоляция коллекций.

Levara оптимизирована для production: строгие SLA на задержку, конкурентные пользователи, микросервисные архитектуры.
...
| Восстановление после сбоя | **100%** | N/A | Levara |

При 100K векторах LanceDB превышает 200 мс на запрос и становится непригодным для real-time сценариев. Levara остаётся в пределах 25 мс благодаря HNSW с O(log N) обходом графа.

### Масштабирование: 1K → 100K векторов

| Масштаб | Levara p50 | LanceDB p50 | Преимущество | Levara QPS | LanceDB QPS |
|---------|--------------|-------------|-------------|-------------|-------------|
| 1 000 | **0.99 мс** | 9.81 мс | **9.9x** | **589** | 98 |
| 10 000 | **7.88 мс** | 55.83 мс | **7.1x** | **480** | 20 |
| 100 000 | **23.66 мс** | 203.71 мс | **8.6x** | **143** | 5 |

Levara масштабируется как O(log N) (HNSW граф). LanceDB деградирует как O(N) (сканирование Arrow файлов). При 100K векторах LanceDB **непригоден для real-time** (>200 мс), Levara остаётся под 25 мс.

## Почему Levara

### Когда Levara — лучший выбор

| Сценарий | Рекомендация | Почему |
|----------|-------------|--------|
| RAG API для 100+ пользователей | Levara | 5.4x выше QPS, Go goroutines, sub-3ms latency |
| Поиск по 100K+ векторам в реальном времени | Levara | 8.6x быстрее, LanceDB >200 мс (непригоден) |
| Микросервисная архитектура (K8s) | Levara | gRPC API, Prometheus metrics, 100% crash recovery |
| SLA < 10 мс на поиск | Levara | p50=2.6 мс, p95=3.5 мс — запас для LLM-обработки |
| Высокая конкурентность (чатбот, multi-user) | Levara | Go goroutines + RWMutex, нет GIL |
...
```
new_db/
  Levara/                   # Go-сервер (основан на VectraDB engine)
    ...
    proto/levara.proto      # Определение gRPC-сервиса
  tests/                      # Python-тесты (174 теста)
  LevaraAdapter.py          # Python gRPC-адаптер
...
Levara настраивается через CLI-флаги (задаются в `docker-compose.yml`):
...
Переменные окружения: `LEVARA_DIM`, `LEVARA_SHARDS`, `HNSW_M`, `HNSW_EF_MULT`, `HNSW_EF_MIN`.

| Сервис | Порт | Назначение |
|--------|------|-----------|
| Levara gRPC | 50051 | Основной API |
| Levara HTTP | 8080 | Prometheus метрики (`/metrics`) |
