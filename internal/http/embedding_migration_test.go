...
func TestEmbeddingMigrationCutoverPromotesShadowAndArchivesLive(t *testing.T) {
	...
	status, cutover := postMigrationJSON(t, app, "/api/v1/embedding-migrations/"+runID+"/cutover", embeddingMigrationCutoverRequest{
		ArchiveCollection: "live__archive_test",
		RetentionDays:     3,
	})
	if status != 200 {
		t.Fatalf("cutover status=%d body=%v", status, cutover)
	}
	if got, _ := cutover["status"].(string); got != "CUTOVER_COMPLETED" {
		t.Fatalf("cutover status=%q body=%v", got, cutover)
	}
	liveMeta := cm.GetMeta("live")
	if liveMeta == nil || liveMeta.EmbeddingModel != "shadow-model" {
		t.Fatalf("live meta after cutover=%+v, want shadow-model", liveMeta)
	}
	archiveMeta := cm.GetMeta("live__archive_test")
	if archiveMeta == nil || archiveMeta.ArchivedFrom != "live" || archiveMeta.RetentionUntil == "" {
		t.Fatalf("archive meta=%+v, want archived_from and retention", archiveMeta)
	}
	restored := getMigrationStatus(t, app, runID)
	if got, _ := restored["status"].(string); got != "CUTOVER_COMPLETED" {
		t.Fatalf("run status=%v body=%v", got, restored)
	}
}
