...
func projectMetaDB(t *testing.T) *sql.DB {
	...
	for _, stmt := range []string{
		...
		`CREATE TABLE data (id TEXT PRIMARY KEY, name TEXT DEFAULT '', pipeline_status TEXT DEFAULT '', source_revision INTEGER DEFAULT 1, raw_content_hash TEXT DEFAULT '', created_at TEXT DEFAULT '')`,
		`CREATE TABLE dataset_data (dataset_id TEXT, data_id TEXT)`,
		`CREATE TABLE document_pipeline_statuses (dataset_id TEXT, data_id TEXT, collection_name TEXT, source_revision INTEGER, raw_content_hash TEXT, pipeline_state TEXT, status_json TEXT, PRIMARY KEY(dataset_id,data_id,collection_name))`,
