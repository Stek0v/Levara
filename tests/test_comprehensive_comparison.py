"""
Comprehensive VectraDB vs LanceDB comparison: 10-step benchmark.
...
async def vectra_insert(session: aiohttp.ClientSession, records: List[Dict]) -> Dict:
    async with session.post(f"{VECTRA_URL}/api/v1/batch_insert", json={"records": records}) as r:
        r.raise_for_status()
        return await r.json()


async def vectra_search(session: aiohttp.ClientSession, vector: List[float],
                        k: int = K) -> List[Dict]:
...
async def vectra_delete(session: aiohttp.ClientSession, ids: List[str]) -> Dict:
    async with session.post(f"{VECTRA_URL}/api/v1/delete", json={"ids": ids}) as r:
        r.raise_for_status()
        return await r.json()


async def vectra_info(session: aiohttp.ClientSession) -> Dict:
    async with session.get(f"{VECTRA_URL}/api/v1/info") as r:
        r.raise_for_status()
        return await r.json()


async def ensure_vectra_healthy(max_wait: int = 30) -> bool:
    """Check VectraDB health, restart if needed. Returns True if healthy."""
    # First check if already healthy
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{VECTRA_URL}/api/v1/info",
...
async def vectra_search_safe(session: aiohttp.ClientSession, vector: List[float],
                              k: int = K) -> List[Dict]:
    """Search with error handling — returns empty list on connection error."""
    try:
        return await vectra_search(session, vector, k)
    except (aiohttp.ClientError, ConnectionError):
        return []


def _extract_text(result: Dict) -> str:
    """Extract text from VectraDB or LanceDB result metadata/payload."""
...
class TestComprehensiveComparison:
    """10-step comprehensive benchmark: VectraDB vs LanceDB."""
    ...
    async def test_02_insert_throughput(self, book_data):
        """Step 2: Insert throughput — VectraDB (HTTP) vs LanceDB (in-process)."""
        ...
        self.__class__._insert_results = {
            "vectra_dps": v_dps, "lance_dps": l_dps,
            "vectra_ms": t_v * 1000, "lance_ms": t_l * 1000,
        }

        print(f"\n  {'Provider':<30} {'dp/s':>8} {'total ms':>10} {'p50 ms':>8} {'p95 ms':>8} {'p99 ms':>8}")
        print(f"  {'-'*75}")
        print(f"  {'VectraDB':<30} {v_dps:>8,.0f} {t_v*1000:>10.1f} {v_stats['p50']:>8.1f} {v_stats['p95']:>8.1f} {v_stats['p99']:>8.1f}")
    ...
    async def test_03_search_latency(self, book_data):
        ...
        print(f"  {'-'*75}")
        print(f"  {'VectraDB':<30} {v_stats['p50']:>7.1f} {v_stats['p95']:>7.1f} {v_stats['p99']:>7.1f} {v_stats['mean']:>7.1f} {v_stats['max']:>7.1f}")
        ...
        winner = "VectraDB" if v_stats['mean'] < l_stats['mean'] else "LanceDB"
    ...
    async def test_04_concurrent_qps(self, book_data):
        ...
        self.__class__._qps_results = {
            "vectra_qps": v_mean_qps, "lance_qps": l_mean_qps,
            "vectra_ms": v_mean_ms, "lance_ms": l_mean_ms,
        }

        print(f"\n  {'Provider':<30} {'avg QPS':>10} {'avg round ms':>14} {'best QPS':>10}")
        print(f"  {'-'*68}")
        print(f"  {'VectraDB':<30} {v_mean_qps:>10,.0f} {v_mean_ms:>14.1f} {max(v_qps):>10,.0f}")
        ...
        winner = "VectraDB" if v_mean_qps > l_mean_qps else "LanceDB"
        print(f"\n  Winner: {winner} ({ratio:.1f}x)")
    ...
    async def test_05_keyword_hit_rate(self, book_data):
        ...
        self.__class__._hit_rate_results = {
            "vectra_rate": v_rate, "lance_rate": l_rate,
            "vectra_hits": v_hits, "lance_hits": l_hits,
        }

        print(f"\n  {'Query':<30} {'VectraDB':>10} {'LanceDB':>10}")
        print(f"  {'-'*52}")
    ...
    async def test_06_ranking_quality(self, book_data):
        ...
        results = {
            "vectra_ndcg": statistics.mean(v_ndcgs),
            "lance_ndcg": statistics.mean(l_ndcgs),
            "vectra_mrr": statistics.mean(v_mrrs),
            "lance_mrr": statistics.mean(l_mrrs),
            "vectra_recall1": statistics.mean(v_recall1),
            "vectra_recall5": statistics.mean(v_recall5),
            "vectra_recall10": statistics.mean(v_recall10),
        ...
        print(f"  {'-'*52}")
        print(f"  {'NDCG@10':<30} {results['vectra_ndcg']:>10.4f} {results['lance_ndcg']:>10.4f}")
        print(f"  {'MRR':<30} {results['vectra_mrr']:>10.4f} {results['lance_mrr']:>10.4f}")
        print(f"  {'Recall@1':<30} {results['vectra_recall1']:>10.4f} {results['lance_recall1']:>10.4f}")
        print(f"  {'Recall@5':<30} {results['vectra_recall5']:>10.4f} {results['lance_recall5']:>10.4f}")
        print(f"  {'Recall@10':<30} {results['vectra_recall10']:>10.4f} {results['lance_recall10']:>10.4f}")

        # Per-query breakdown
        print(f"\n  Per-query NDCG@10:")
        print(f"  {'Query':<30} {'VectraDB':>10} {'LanceDB':>10}")
        print(f"  {'-'*52}")
        for i, cq in enumerate(QUERIES):
    ...
    async def test_07_concurrent_read_write(self, book_data):
        ...
        # ── VectraDB: search with concurrent writes ──
    ...
    async def test_08_multi_collection_isolation(self, book_data):
        ...
        # ── VectraDB: insert with collection prefixes ──
        async with aiohttp.ClientSession() as session:
        ...
        # ── VectraDB: search and check isolation ──
        leakage_count = 0
        total_checks = 0
        ...
        async with aiohttp.ClientSession() as session:
            for col_name, col_chunks in collections.items():
                ...
                results = await vectra_search(session, vecs[query_idx], k=20)
        ...
        self.__class__._isolation_results = {
            "vectra_leakage": leakage_count,
            "vectra_total": total_checks,
        ...
        print(f"  {'-'*65}")
        print(f"  {'VectraDB (prefix-based)':<30} {leakage_count:>10} {total_checks:>14} {leak_rate_v:>7.1f}%")
        print(f"  {'LanceDB (separate tables)':<30} {lance_leakage:>10} {lance_total:>14} {leak_rate_l:>7.1f}%")

        if leakage_count > 0:
            print(f"\n  WARNING: VectraDB cross-collection leakage detected!")
            print(f"  VectraDB uses prefix-based filtering, not physical isolation.")
    ...
    async def test_09_scale_test(self, book_data):
        ...
        # Ensure VectraDB is healthy (may have crashed from accumulated data)
        print(f"  Checking VectraDB health...")
        if not await ensure_vectra_healthy(max_wait=30):
            pytest.skip("VectraDB not available for scale test")
        ...
        print(f"  {'-'*52}")
        print(f"  {'VectraDB':<30} {v_insert_dps:>10,.0f} {t_v_insert:>10.1f}")
        ...
        print(f"  {'-'*65}")
        print(f"  {'VectraDB':<30} {v_search_stats['p50']:>7.1f} {v_search_stats['p95']:>7.1f} {v_search_stats['p99']:>7.1f} {v_search_stats['mean']:>7.1f}")
        ...
        print(f"  {'-'*42}")
        print(f"  {'VectraDB':<30} {v_recall_mean:>10.4f}")
        ...
        self.__class__._scale_results = {
            "n_vectors": N_scale,
            "vectra_insert_dps": v_insert_dps,
            "lance_insert_dps": l_insert_dps,
            "vectra_search_mean": v_search_stats["mean"],
            "lance_search_mean": l_search_stats["mean"],
            "vectra_recall10": v_recall_mean,
        ...
        if baseline_latency and baseline_insert:
            ...
            v_tp_change = ((v_insert_dps - baseline_insert.get("vectra_dps", 0))
                           / max(baseline_insert.get("vectra_dps", 1), 1)) * 100

            print(f"\n  Scale vs baseline ({N_base} → {N_scale} vectors, {MULTIPLIER}x):")
            print(f"  VectraDB latency change: {v_lat_growth:+.1f}%")
            print(f"  VectraDB throughput change: {v_tp_change:+.1f}%")

    # ── Step 10: Crash Recovery (VectraDB only) ───────────────────────────
    ...
    async def test_10_crash_recovery(self, book_data):
        ...
        print("\n" + "=" * 72)
        print(f"  STEP 10: CRASH RECOVERY (VectraDB only)")
        print("=" * 72)

        # Ensure VectraDB is healthy first
        if not await ensure_vectra_healthy(max_wait=30):
            self.__class__._crash_results = {"status": "SKIP", "reason": "VectraDB not available"}
            pytest.skip("VectraDB not available for crash recovery test")
        ...
        async with aiohttp.ClientSession() as session:
            pre_results = await vectra_search(session, vecs[0], k=50)
        ...
        # Restart VectraDB container
        print(f"  Restarting VectraDB container...")
        cwd = str(Path(__file__).parent.parent)
        restart_ok = False
        try:
            result = subprocess.run(
                ["docker", "compose", "restart", "vectradb"],
                capture_output=True, text=True, timeout=60,
                cwd=cwd,
            )
            if result.returncode != 0:
                for name in ["new_db-vectradb-1", "new-db-vectradb-1"]:
                    result = subprocess.run(
                        ["docker", "restart", name],
                        capture_output=True, text=True, timeout=60,
                    )
        ...
        if not restart_ok:
            ...
            pytest.skip("Could not restart VectraDB container")

        # Wait for VectraDB to come back up (up to 60s)
        print(f"  Waiting for VectraDB to recover...")
        recovered = False
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if r.status == 200:
                            info = await r.json()
                            print(f"  VectraDB back online (attempt {attempt + 1}): {info}")
                            recovered = True
                            break
            except Exception:
                pass
        ...
        if not recovered:
            self.__class__._crash_results = {"status": "FAIL", "reason": "VectraDB did not recover in 90s"}
            pytest.fail("VectraDB did not come back online after restart")
        ...
        async with aiohttp.ClientSession() as session:
            for i in range(0, min(10, len(vecs))):
                results = await vectra_search_safe(session, vecs[i], k=50)
                if not results and i == 0:
                    print(f"  WARNING: VectraDB crashed during post-restart search!")
        ...
        if crashed:
            self.__class__._crash_results = {
                ...
                "found": 0,
                "recovery_rate": 0,
                "note": "VectraDB panics on search after WAL recovery (DiskStore.Read makeslice)",
            }
            print(f"\n  {'Metric':<30} {'Value':>10}")
            print(f"  {'-'*42}")
    ...
    async def test_11_summary(self, book_data):
        ...
        print("\n")
        print("=" * 80)
        print("  COMPREHENSIVE COMPARISON: VectraDB vs LanceDB")
        print(f"  Book: «Ураган» (Джанет Эдвардс), {len(chunks)} base chunks, dim={DIM}")
        print(f"  Embeddings: pplx-embed-context-v1-0.6b (real)")
        print("=" * 80)

        print(f"\n  {'#':<4} {'Test':<35} {'VectraDB':>12} {'LanceDB':>12} {'Winner':>10}")
        print(f"  {'-'*75}")

        # Step 2: Insert
        ir = self.__class__._insert_results
        if ir:
            v_val = f"{ir.get('vectra_dps', 0):,.0f} dp/s"
            l_val = f"{ir.get('lance_dps', 0):,.0f} dp/s"
            winner = "VectraDB" if ir.get('vectra_dps', 0) > ir.get('lance_dps', 0) else "LanceDB"
        ...
        if lr:
            ...
            winner = "VectraDB" if lr.get('vectra', {}).get('mean', 999) < lr.get('lance', {}).get('mean', 999) else "LanceDB"
            print(f"  {'3':<4} {'Search latency (mean)':<35} {v_val:>12} {l_val:>12} {winner:>10}")

        # Step 4: QPS
        qr = self.__class__._qps_results
        if qr:
            v_val = f"{qr.get('vectra_qps', 0):,.0f} qps"
            l_val = f"{qr.get('lance_qps', 0):,.0f} qps"
            winner = "VectraDB" if qr.get('vectra_qps', 0) > qr.get('lance_qps', 0) else "LanceDB"
            print(f"  {'4':<4} {'Concurrent QPS':<35} {v_val:>12} {l_val:>12} {winner:>10}")

        # Step 5: Hit rate
        hr = self.__class__._hit_rate_results
        if hr:
            v_val = f"{hr.get('vectra_rate', 0):.0%}"
            l_val = f"{hr.get('lance_rate', 0):.0%}"
            winner = "VectraDB" if hr.get('vectra_rate', 0) >= hr.get('lance_rate', 0) else "LanceDB"
            print(f"  {'5':<4} {'Keyword hit rate':<35} {v_val:>12} {l_val:>12} {winner:>10}")

        # Step 6: Ranking
        rr = self.__class__._ranking_results
        if rr:
            v_val = f"{rr.get('vectra_ndcg', 0):.4f}"
            l_val = f"{rr.get('lance_ndcg', 0):.4f}"
            winner = "VectraDB" if rr.get('vectra_ndcg', 0) >= rr.get('lance_ndcg', 0) else "LanceDB"
            print(f"  {'6':<4} {'NDCG@10':<35} {v_val:>12} {l_val:>12} {winner:>10}")

            v_val = f"{rr.get('vectra_mrr', 0):.4f}"
            l_val = f"{rr.get('lance_mrr', 0):.4f}"
            winner = "VectraDB" if rr.get('vectra_mrr', 0) >= rr.get('lance_mrr', 0) else "LanceDB"
            print(f"  {'6':<4} {'MRR':<35} {v_val:>12} {l_val:>12} {winner:>10}")

            v_val = f"{rr.get('vectra_recall10', 0):.4f}"
            l_val = f"{rr.get('lance_recall10', 0):.4f}"
            winner = "VectraDB" if rr.get('vectra_recall10', 0) >= rr.get('lance_recall10', 0) else "LanceDB"
        ...
        if iso:
            v_leak = iso.get("vectra_leakage", 0)
        ...
        if sr:
            ...
            v_val = f"{sr.get('vectra_search_mean', 0):.1f} ms"
            l_val = f"{sr.get('lance_search_mean', 0):.1f} ms"
            winner = "VectraDB" if sr.get('vectra_search_mean', 999) < sr.get('lance_search_mean', 999) else "LanceDB"
