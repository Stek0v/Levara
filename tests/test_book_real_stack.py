...
This test bypasses Cognee and talks directly to:
  ...
  2. VectraDB — for real vector insert/search

This way we measure the actual VectraDB + real embedding pipeline performance.
...
def _stack_available():
    return (
        ...
        and _check_service("http://localhost:8080/metrics", "VectraDB")
    )
...
async def vectra_insert(
    ...
    """Batch insert into VectraDB."""
    payload = {"records": records}
    async with session.post(
        f"{VECTRA_URL}/api/v1/batch_insert",
        json=payload,
    ) as resp:
        if resp.status == 404:
...
class TestRealStackInsert:
    """Insert real book data into real VectraDB with real embeddings.
    ...
    async def test_full_insert(self, book_chunks):
        print("\n" + "═" * 70)
        print("  REAL STACK INSERT — «Ураган» → embed-server → VectraDB")
...
class TestRealStackSearch:
    """Search with real embeddings and real VectraDB."""
    ...
    async def test_search_quality_and_latency(self, book_chunks):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for start in range(0, len(records), 50):
                await vectra_insert(session, records[start:start + 32])
            ...
            print(f"\n  {'─' * 70}")
            print(f"\n  Search Latency (VectraDB, {N} vectors, real HNSW):")
            ...
            for _ in range(5):
                for qvec in query_vectors:
                    t0 = time.perf_counter()
                    await vectra_search(session, qvec, k=10)
            ...
            await asyncio.gather(*[
                vectra_search(session, qv, k=10) for qv in query_vectors
            ])
...
class TestRealStackReport:
    ...
    async def test_full_report(self, book_chunks):
        ...
        print("  Qwen3.5-27B + pplx-embed-v1-0.6b + VectraDB (dim=1024)")
        print("═" * 70)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _ in range(5):
                for qv in query_vectors:
                    t0 = time.perf_counter()
                    await vectra_search(session, qv, k=10)
            ...
            # QPS
            t0 = time.perf_counter()
            await asyncio.gather(*[vectra_search(session, qv, k=10) for qv in query_vectors])
            t_conc = time.perf_counter() - t0
            qps = len(query_vectors) / t_conc
            ...
            for cq, qv in zip(CONTEXTUAL_QUERIES, query_vectors):
                results = await vectra_search(session, qv, k=10)
                combined = " ".join(
  ...
  │ VectraDB shards                        │ {vectra_info.get('shards', '?'):<13} │
  ├────────────────────────────────────────┼───────────────┤
  │ Embed time ({N} chunks)               │ {embed_time*1000:<10.0f} ms │
  │ Embed throughput                       │ {N/embed_time:<10.0f} /s │
  │ VectraDB insert time                   │ {insert_time*1000:<10.0f} ms │
  │ VectraDB insert throughput             │ {N/insert_time:<10.0f} /s │
  ...
  Stack: Qwen3.5-27B (Q4_K_M) + pplx-embed-context-v1 + VectraDB
  GPU: RTX 3090 (CUDA)
""")
        print("═" * 70)
