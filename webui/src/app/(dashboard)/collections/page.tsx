...
import { useCollections } from '@/hooks/use-levara'
  ...
  return (
    <div>
      ...
      {collections.length === 0 ? (
        <EmptyState icon={FolderOpen} title="No collections" description="Collections are created automatically when you upload and process data" />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {collections.map((c) => (
            <div key={c.name} className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                <p className="text-xs text-gray-400 truncate" title={c.embedding_model}>Model: {c.embedding_model || 'default'}</p>
                {c.domain && <Badge variant="info" className="mt-1">{c.domain}</Badge>}
              </div>
            </div>
          ))}
        </div>
